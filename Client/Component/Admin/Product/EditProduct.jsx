...
function EditProduct() {
    ...
    useEffect(() => {
        if (navigate.query.proId) {
            adminAxios((server) => {
                server.get('/admin/getOneProduct/' + navigate.query.proId).then((product) => {
                    if (product.data.login) {
                        logOut()
                    } else {
                        setProductDetails(product.data)
                        var files = product.data.files
                        var ImagesServer = files.map((ele, key) => {
                            var location = ServerId + '/product/' + product.data.uni_id_1 + product.data.uni_id_2 + '/' + files[key].filename
                            return location
                        })
    ...
    function FormSubmit(e) {
        ...
        adminAxios((server) => {
            server.put('/admin/editProduct/' + navigate.query.proId, formData, {
                headers: {
